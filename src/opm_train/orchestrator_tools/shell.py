...
class ShellToolMixin:
    ...
    async def _tool_shell(self, run: ToolRun, action: dict[str, Any]) -> dict[str, Any]:
        ...
        timeout_seconds = max(1.0, float(self.config.runtime.tools.shell_timeout_seconds))
        ...
        async def worker() -> dict[str, Any]:
            ...
            try:
                ...
                timed_out_result = publish(
                    exit_code=int(process.returncode or -1) if process is not None else -1,
                    timed_out=True,
                )
        ...
        try:
            ...
            return {
                ...
                **({"timed_out": True, "timeout_seconds": timeout_seconds} if bool(result.get("timed_out")) else {}),
            }
        except asyncio.TimeoutError:
            running = run.result if isinstance(run.result, dict) else {}
            return _running_payload(run_id=run.id, result=running)
        except Exception:
            task_observed = True
            raise


def _running_payload(*, run_id: str, result: dict[str, Any]) -> dict[str, Any]:
